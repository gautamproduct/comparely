...
// Strategy: Open blinkit.com on a mobile UA with geolocation set, search for the query,
...
export async function scrapeBlinkit(
  ...
  try {
    ...
    // Land on home, let location resolve
    ...
    // Extract product data from page state or DOM
    const products = await page.evaluate(() => {
      ...
      // Blinkit renders product cards with data attributes. We try multiple selectors.
      const cards = document.querySelectorAll(
        '[data-test-id*="plp-product"], [class*="ProductCard"], [class*="plp-product"]',
      );
